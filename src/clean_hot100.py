...
def clean_hot100_song() -> pd.DataFrame:
    ...
    hot100_song_df = (
        hot100_song_df
        .groupby(["title", "performer", "chart_week"], as_index=False) # sort=True by default
        .agg(
...
def filter_hot100_song(hot100_song_df: pd.DataFrame, start_wk: pd.Timestamp =START_WEEK, end_wk: pd.Timestamp =END_WEEK) -> pd.DataFrame:
    ...
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    filtered_hot100_song_df.to_parquet(OUTPUT_DIR / "hot-100-song_{start_wk:%Y%m%d}_{end_wk:%Y%m%d}.parquet", index=False)
...
def main():
    ...
    print(f"{len(filtered_hot100_song_df)} songs on Billboard Hot-100 from {start_wk:%Y%m%d} to {end_wk:%Y%m%d}.")
